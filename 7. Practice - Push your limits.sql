...
with cte as
(select *,
dense_rank() over (partition by username order by startDate desc) as rec,
count(*) over (partition by username) as cnt
from UserActivity)
select * from cte
where rec = 2 or cnt = 1;
-------------------------------------------------------------
...
with cte as(
select *,
lag(marks) over (partition by student_id ) as lag_marks
from exams
...
with cte as 
(SELECT *,
cast(dense_rank() OVER (PARTITION BY city ORDER BY days) as inc_days) - 
cast(dense_rank() OVER (PARTITION BY city ORDER BY cases) as inc_cases) as diff
FROM covid)
...
-- Write a SQL query to get the percentage of students who scored 90 or above in any subject 
    ...
    select 100 *
    count(distinct case
        when marks >= 90 then studentid else null end) / count(distinct studentid)  as new_col
        from students;
...
with cte as (select *,
dense_rank() over (partition by subject order by marks desc) as highest_marks,
dense_rank() over (partition by subject order by marks ) as lowest_marks
from students)
select *
from cte
where highest_marks =2 or lowest_marks = 2;
